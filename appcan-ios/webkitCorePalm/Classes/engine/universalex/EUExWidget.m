...
-(void)startWidget:(NSMutableArray *)inArguments {
    ...
    if(wgtObj){
        ...
        Class  uexDataAnalysisClass =  NSClassFromString(@"UexDataAnalysisAppCanAnalysis");//判断类是否存在，如果存在子widget上报
        
        if (uexDataAnalysisClass) {
            ...
            id analysisObject = [[uexDataAnalysisClass alloc]init];
...
#pragma clang diagnostic pop
        ...
        if (analysisClass) {
            //过滤掉无appkey的子应用上报(plugin类型)
            NSLog(@"4.0添加AppCanAnalysis进行子应用上报");
            if (eBrwWndContainer.mwWgt.wgtType == F_WWIDGET_PLUGINWIDGET) return;
...
-(void)finishWidget:(NSMutableArray *)inArguments {
    ...
    if ([inArguments count] > 3) {
        ...
        animiId = [[inArguments objectAtIndex:3] intValue];
        
    } else {
        
        animiId = [BAnimation ReverseAnimiId:eBrwWndContainer.mStartAnimiId];
...
- (void)sendReportRead:(id)userInfo {
    ...
    NSString *urlString =[NSString stringWithFormat:@"%@4.0/count/",theApp.useBindUserPushURL];
    
    
    if ([dictionary objectForKey:@"taskId"]) {
    ...
    if ([dictionary objectForKey:@"tenantId"]) {
    ...
    NSString *softTokenStr = [EUtility md5SoftToken];
    ...
    [bodyDict setObject:softTokenStr forKey:@"softToken"];
    ...
    [postData setObject:[dictionary objectForKey:@"msgId"] forKey:@"msgId"];
    ...
    ASIFormDataRequest *asiRequest = [[ASIFormDataRequest alloc] initWithURL:requestUrl];
    [asiRequest setRequestMethod:@"POST"];
    [asiRequest setPostValue:[dictionary objectForKey:@"msgId"] forKey:@"msgId"];
    [asiRequest setPostValue:softToken forKey:@"softToken"];
    [asiRequest setPostValue:@"open" forKey:@"eventType"];
    [asiRequest setPostValue:timeSp forKey:@"occuredAt"];
    ...
    int status = asiRequest.responseStatusCode;
    if (status == 200) {
...
-(void)setPushInfo:(NSMutableArray *)inArguments {
    ...
    NSLog(@"appcan-->AppCanEngine-->EUExWidget-->setPushInfo-->inArguments == %@",inArguments);
    ...
    if ([theApp.useBindUserPushURL rangeOfString:@"push"].location == NSNotFound) {
        ...
        [requestSetPushInfo setCompletionBlock:^{
            ...
            if (200 == requestSetPushInfo.responseStatusCode) {
                ...
                NSLog(@"appcan-->Engine-->EUExWidget.m-->delPushInfo-->request.responseString is %@",requestSetPushInfo.responseString);
                
                
            } else {
                
                NSLog(@"appcan-->Engine-->EUExWidget.m-->delPushInfo-->request.responseStatusCode is %d--->[request error] = %@",requestSetPushInfo.responseStatusCode, [requestSetPushInfo error]);
        ...
        [requestSetPushInfo setFailedBlock:^{
            ...
            NSLog(@"appcan-->Engine-->EUExWidget.m-->delPushInfo-->setFailedBlock-->error is %@",[requestSetPushInfo error]);
...
- (void)sendPushUserMsg:(id)userInfo {
    ...
    if ([theApp.useBindUserPushURL rangeOfString:@"push"].location == NSNotFound) {
        ...
        NSLog(@"appcan-->Engine-->EUExWidget.m-->sendPushUserMsg-->theApp.validatesSecureCertificate is %d-->theApp.useCertificateControl = %d",theApp.validatesSecureCertificate,theApp.useCertificateControl);
        ...
        [requestSetPushInfo setCompletionBlock:^{
            ...
            if (200 == requestSetPushInfo.responseStatusCode) {
                ...
                NSLog(@"appcan-->Engine-->EUExWidget.m-->sendPushUserMsg-->request.responseString is %@",requestSetPushInfo.responseString);
                
                
            } else {
                
                NSLog(@"appcan-->Engine-->EUExWidget.m-->sendPushUserMsg-->request.responseStatusCode is %d--->[request error] = %@",requestSetPushInfo.responseStatusCode, [requestSetPushInfo error]);
                
            }
        }];
        [requestSetPushInfo setFailedBlock:^{
            @strongify(requestSetPushInfo);
            NSLog(@"appcan-->Engine-->EUExWidget.m-->sendPushUserMsg-->setFailedBlock-->error is %@",[requestSetPushInfo error]);
        ...
        @autoreleasepool {
            urlStr = [NSString stringWithFormat:@"%@msg/%@/bindUser",theApp.useBindUserPushURL,softToken];
            SecIdentityRef identity = NULL;
            SecTrustRef trust = NULL;
            SecCertificateRef certChain=NULL;
            NSData *PKCS12Data = [NSData dataWithContentsOfFile:[BUtility clientCertficatePath]];
            ...
            if (theApp.useCertificateControl) {
                ...
                [FormatReq setClientCertificateIdentity:nil];
            }
